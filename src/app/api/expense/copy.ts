...
export async function PUT(
  ...
  try {
    ...
    // ATUALIZA UMA DESPESA NÃO RECORRENTE PARA UMA DESPESA RECORRENTE
    if (
      data.recurrence_type_id === 1 &&
      existingExpense.recurrence_type_id === 2
    ) {
      ...
      const recurrenceData = generateRecurrences(
        startDate,
        recurrenceAmount || Number(existingExpense.amount),
        Number(id)
      );
    ...
    // ATUALIZA UMA DESPESA PARCELADA PARA UMA DESPESA RECORRENTE
    if (
      data.recurrence_type_id === 1 &&
      existingExpense.recurrence_type_id === 3
    ) {
      // Deleta as parcelas existentes da despesa no banco de dados
      await prisma.installmentExpense.deleteMany({
        where: { expense_id: Number(id) },
      });
      ...
      const recurrenceData = generateRecurrences(
        startDate,
        Number(data.amount) || Number(existingExpense.amount),
        Number(id)
      );
    ...
    // ATUALIZA UMA DESPESA RECORRENTE PARA OUTRA DESPESA RECORRENTE
    if (
      data.recurrence_type_id === 1 &&
      existingExpense.recurrence_type_id === 1
    ) {
      // Deleta as recorrências existentes da despesa no banco de dados
      await prisma.recurrenceExpense.deleteMany({
        where: { expense_id: Number(id) },
      });
